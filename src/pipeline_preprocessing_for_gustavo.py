...
def execute_json(source_dir, json_name, testCases, output_only, just_preprocessing):
    ...
    for id,sol in enumerate(solutions): 
        ...
        #     merged_variable_keys = list(set(sequence_comparison[key].keys() + merged_variable_keys))
        ...
        key_set0 = set(sequence_comparison[prog_versions[0]].keys())
        key_set1 = set(sequence_comparison[prog_versions[1]].keys())
        mutual_key_set = key_set0.intersection(key_set1)
        #print 'key_set0',key_set0
        #print 'key_set1',key_set1
        #print 'list(mutual_key_set)',list(mutual_key_set)
        #print 'testCases',testCases
        for var_key in list(mutual_key_set):
            #print var_key
            #print sequence_comparison[prog_versions[0]][var_key]
            #print sequence_comparison[prog_versions[1]][var_key]
            
            if not sequence_comparison[prog_versions[0]][var_key] == sequence_comparison[prog_versions[1]][var_key]:
                sequence_comparison_diff[var_key] = {}
                cases_set0 = set(sequence_comparison[prog_versions[0]][var_key].keys())
                cases_set1 = set(sequence_comparison[prog_versions[1]][var_key].keys())
                mutual_test_cases = cases_set0.intersection(cases_set1)
                for case_key in mutual_test_cases:
                    #print 'case_key',case_key
                    val_seq0 = sequence_comparison[prog_versions[0]][var_key][case_key]
                    val_seq1 = sequence_comparison[prog_versions[1]][var_key][case_key]
                    if not val_seq0 == val_seq1:
                        sequence_comparison_diff[var_key][case_key] = (val_seq0,val_seq1)
                    #if not sequence_comparison[prog_versions[0]][var_key][case] == sequence_comparison[prog_versions[1]][var_key][case]:
                    #    sequence_comparison_diff[var_key][case] = (sequence_comparison[prog_versions[0]][var_key][case],sequence_comparison[prog_versions[1]][var_key][case])
        pprint.pprint(sequence_comparison_diff)
        sol['sequence_comparison_diff'] = sequence_comparison_diff
